...
try:
  ...
  if (!continuous_read) {
    // whole file read mode may cause long wait time before start of playback, so display "Now loading" message
    _iocs_b_print(cp932rsc_now_loading);
  }
  if (staging_file_read) {
    // ハイメモリへのダイレクトロードが不可能な場合は、一旦メインメモリを経由する
